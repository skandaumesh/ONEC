...
export default function Navbar() {
 ...
 <div className="flex items-center gap-4">
 <Link href="/prescription-upload" className="hover:underline flex items-center gap-1">
 ...
 router.push(`/products?search=${encodeURIComponent(item.query)}`);
 ...
 whileTap={{ scale: 0.95 }}
 onClick={() => isAuthenticated ? setCartOpen(true) : router.push('/login')}
 ...
 {categories.map((cat) => (
 <Link key={cat.name} href={cat.path} onClick={() => setMobileMenuOpen(false)}
