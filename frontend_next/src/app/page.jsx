...
const HomePage = () => {
 ...
 <motion.button
 key={idx}
 onClick={() => router.push(`/products?search=${encodeURIComponent(item.symptom)}`)}
 initial={{ opacity: 0, y: 15 }}
 animate={{ opacity: 1, y: 0 }}
 ...
 <button onClick={() => router.push('/products')} className="text-sm font-bold text-[var(--color-primary)] flex items-center gap-1 hover:gap-2 transition-all cursor-pointer">
 ...
 <motion.button
 key={idx}
 onClick={() => router.push(`/products?search=${encodeURIComponent(con.query)}`)}
 initial={{ opacity: 0, y: 10 }}
 animate={{ opacity: 1, y: 0 }}
