...
import { Search, Star, ShoppingCart, ChevronLeft, ChevronRight, X, SlidersHorizontal, Grid3X3, LayoutGrid, List, Eye, Pill, Plus, Sparkles } from 'lucide-react';
import { productApi, categoryApi } from '@/api';
import { useAuth } from '@/context/AuthContext';
import { useCart } from '@/context/CartContext';
import { AIPopup } from '@/components/common/AIPopup';
import { AIComparePopup } from '@/components/common/AIComparePopup';
...
export default function ProductsPage() {
 ...
 const [activeAiProduct, setActiveAiProduct] = useState(null);
 ...
 <div className="w-10 h-10 rounded-xl bg-[#FFF3F2] flex items-center justify-center border border-[#FF6F61]/10 shadow-sm">
 <Pill size={20} className="text-[#FF6F61]" />
 </div>
 ...
 className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${viewMode === mode ? 'bg-[#FF6F61] text-white shadow' : 'bg-white/80 backdrop-blur border border-slate-200/50 text-slate-500 hover:border-[#FF6F61]/30 hover:text-[#FF6F61] cursor-pointer'}`}>
 <Icon size={16} />
 </button>
 ))}
 </div>
 ...
 <div className={`${filterOpen ? 'absolute right-0 top-0 h-full w-72 p-6 bg-[var(--bg-surface)] border-l border-slate-200/50 ' : 'sticky top-24 glass-blur border border-slate-200/50 '} rounded-3xl p-5 shadow-md`}>
 {filterOpen && (
 ...
 <div className="absolute top-3 left-3 z-20">
   <input 
     type="checkbox" 
     checked={selectedForCompare.some(p => p.id === product.id)}
     onChange={(e) => handleCompareToggle(product, e.target.checked)}
     className="w-5 h-5 rounded border-gray-300 text-purple-600 focus:ring-purple-600 cursor-pointer shadow-sm"
 ...
 <span className="absolute top-2.5 left-10 px-2.5 py-1 rounded-full bg-[#EF4444] text-white text-[9px] font-black shadow-md border border-red-500/20">
 ...
 <Link href={`/products/${product.id}`} className="hover:text-[var(--color-primary)] transition-colors">{product.name}</Link>
 </h3>
 ...
 className="flex-1 py-2.5 px-3 border-2 border-[#FF6F61] text-[#FF6F61] hover:bg-[#FF6F61] hover:text-white rounded-xl text-xs font-black transition-all cursor-pointer flex items-center justify-center gap-1.5 bg-white btn-shine"
 >
 ADD <Plus size={12} />
 </motion.button>
 <motion.button
 whileHover={{ scale: 1.02 }}
 whileTap={{ scale: 0.98 }}
 onClick={() => { setActiveAiProduct(product); setAiPopupOpen(true); }}
 className="w-10 flex-shrink-0 border-2 border-purple-200 text-purple-600 hover:bg-purple-50 rounded-xl flex items-center justify-center transition-all cursor-pointer bg-white shadow-sm"
 ...
 productId={activeAiProduct?.id} 
 productName={activeAiProduct?.name} 
 />

 <AIComparePopup
   isOpen={aiCompareOpen}
 ...
 <AnimatePresence>
   {selectedForCompare.length > 0 && (
     ...
     >
       <div className="bg-white rounded-full shadow-2xl border border-purple-100 p-2 flex items-center gap-4">
         <div className="flex -space-x-2 px-2">
           {selectedForCompare.map((p, i) => (
             <div key={p.id} className={`w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-xs font-bold text-white shadow-sm ${i === 0 ? 'bg-purple-500' : 'bg-indigo-500'}`} title={p.name}>
         ...
         <button
           onClick={() => setAiCompareOpen(true)}
           disabled={selectedForCompare.length !== 2}
           className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 btn-shine"
         >
           <Sparkles size={16} /> Compare
         </button>
         <button
           onClick={() => setSelectedForCompare([])}
           className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors mr-1"
